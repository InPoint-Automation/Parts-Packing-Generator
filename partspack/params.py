...
class TiltMode(str, Enum):
    A = "A"                # extract straight up
    B = "B"                # extract along tilted axis
...
class Params(BaseModel):
    ...
    flip: bool = False
    tilt_deg: float = 0.0
    tilt_axis: Axis = Axis.X
    tilt_mode: TiltMode = TiltMode.A
    ...
    section_deflection: float = 0.05     # mm
    ...
    pocket_rotate_deg: float = 180.0
    pocket_rotate_pattern: PocketRotate = PocketRotate.ALT_COLS
    ...
    outside_style: Optional[SkeletonStyle] = None   # deprecated
    outside_through: bool = False
    ...
    fast_base_boolean: bool = False      # experimental; OCCT can segfault
    ...
    divot_strategy: DivotStrategy = DivotStrategy.PERIMETER
    push_hole: bool = True
    ...
    def _clamp(self) -> "Params":
        ...
        _set("section_deflection", max(0.001, float(self.section_deflection)))
        _set("min_island_area", max(0.0, float(self.min_island_area)))
        _set("smooth_iters", max(0, int(self.smooth_iters)))
